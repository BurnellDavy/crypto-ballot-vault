...
export function Profile() {
  ...
  const { votes, userVotes, decryptedUserVotes, loadVotes, contractAddress } = useVoting();
  const [voteStats, setVoteStats] = useState<Record<number, { voteCount: number; participants: number }>>({});

  // Refresh data periodically when connected (initial load is handled by the hook)
  useEffect(() => {
